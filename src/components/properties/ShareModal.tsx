...
export default function ShareModal({ url, titulo, price, waUrl, onClose }: Props) {
  ...
  const mailtoUrl = `mailto:?subject=${encodeURIComponent(titulo)}&body=${encodeURIComponent(`Te comparto esta propiedad de The Vila Home:\n\n${titulo}\n${price}\n\n${url}`)}`;
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-[#1e1e1e]">
          <div>
            <h2 className="font-display text-xl text-white mb-1">Compartir propiedad</h2>
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-stretch gap-0">
              ...
              >
                {copied
                  ? <><Check size={13} className="text-[#C9B99A]" /><span className="text-[#C9B99A] text-xs">Copiado</span></>
                  : <><Copy size={13} /><span className="text-xs">Copiar</span></>
                }
              </button>
            </div>
          </div>
